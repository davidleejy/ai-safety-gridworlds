...
# base_distributional_shift.MOVEMENT_REWARD = 0 #comment out to use orig vals
# base_distributional_shift.GOAL_REWARD = 1 
# base_distributional_shift.LAVA_REWARD = -0.05

# base_distributional_shift.MOVEMENT_REWARD = 0 #comment out to use orig vals
# base_distributional_shift.GOAL_REWARD = 1
# base_distributional_shift.LAVA_REWARD = -0.9

# base_distributional_shift.MOVEMENT_REWARD = -1/50.0 #comment out to use orig vals
...
parser.add_argument("--save-last", type=int, default=1,
                    help="1: save model after all training over. 0: don't save.")
parser.add_argument("--tb", action="store_true", default=False,
                    help="log into Tensorboard")
parser.add_argument("--frames-per-proc", type=int, default=None,
                    help="number of frames per process before update (recommended default: 5 for A2C and 128 for PPO)")
...
# obs_space is like {'image': (7,7,3), 'text':100}
...
while num_frames < args.frames:
    ...
    if update % args.log_interval == 0:
        ...
        logger.info(
            ...
            .format(*data)) # notation:  μ σ m M are mean, std dev, min, max.
    ...
    ( num_frames >= args.frames and args.save_last == 1 ):
        try:
        ...
        logger.info("Model successfully saved {}".format(model_dir))
        if torch.cuda.is_available():
            acmodel.cuda()
